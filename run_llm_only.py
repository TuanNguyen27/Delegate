#!/usr/bin/env python3
"""
Run LLM Baseline Experiment Only (Gemini 2.5 Flash)
This script runs only the LLM baseline on a specific sample set
"""
import asyncio
import argparse
import sys
...
async def main():
    parser = argparse.ArgumentParser(description='Run LLM baseline experiment only')
    ...
    if args.output:
        ...
        output_dir = Path(f"results_llm_only_{args.samples}samples_{timestamp}")
    ...
    print("\n" + "="*80)
    print("EXPERIMENT: LLM BASELINE (Gemini 2.5 Flash Lite)")
    print("="*80)
    
    llm_file = output_dir / "results_llm.json"
    summary = await run_llm_experiment(
        test_df.copy(), 
        str(llm_file),
